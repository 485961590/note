...
```
![](file-20260812155048510.png)

![](file-20260812155048513.png)

![](file-20260812155048514.png)

| 特性             | **原始反弹Shell**          | **升级后的PTY Shell**      |
| -------------- | ---------------------- | ---------------------- |
| **命令历史**       | ❌ 不支持（按上下键无效）          | ✅ 支持（可用上下键翻找历史命令）      |
| **Tab补全**      | ❌ 不支持或异常               | ✅ 正常工作                 |
| **信号处理**       | ❌ Ctrl+C会杀死整个Shell     | ✅ Ctrl+C只中断当前命令        |
| **作业控制**       | ❌ `fg`/`bg`/`jobs`无效   | ✅ 完全支持（可后台运行任务）        |
| **终端颜色**       | ❌ 无颜色，输出混乱             | ✅ 颜色正常（如`ls`的输出）       |
| **文本编辑器**      | ❌ `vim`/`nano`可能无法正常显示 | ✅ 全屏编辑器正常工作            |
| **`sudo`密码提示** | ❌ 无法显示密码输入提示           | ✅ 正常显示并读取密码            |
| **Clear清屏**    | ❌ 无效                   | ✅ `clear`或`Ctrl+L`正常工作 |
