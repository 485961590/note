...
> 来源：[菜鸟教程 · Go 语言文件处理](https://www.runoob.com/go/go-file-handle.html)
>
> 文件处理是 Go 中最常见的操作之一——读取配置、写入日志、数据持久化都离不开它。核心套路：`os` 打开/创建 → 按需用 `bufio`/`io` 读写 → `defer Close()` 收尾。
...
|------|------|
| `os` | 核心：创建、读写、删除、权限 |
| `io` | 通用 Reader/Writer 接口 |
| `bufio` | 缓冲读写，性能优化 |
| `filepath` | 跨平台路径处理 |
| `ioutil` | Go 1.16 已弃用 → `os` / `io` |

> `ioutil` 包在 Go 1.16 已弃用，其功能迁移到 `os`（如 `os.ReadFile`、`os.WriteFile`）和 `io`（如 `io.ReadAll`）。新代码应直接使用 `os` 和 `io` 包。

**典型使用流程：** `os.Open()` → `bufio.Scanner` → 处理数据 → `defer Close()`

---

## 1. 文件创建（os.Create）

`os.Create` 创建新文件；**文件已存在则被截断（清空内容）**。返回的文件对象必须关闭以释放资源：
...
> `defer file.Close()` 是 Go 文件操作的最佳实践：即使后续代码 panic，文件也会被正确关闭，避免文件描述符泄漏。
...
Go 提供三种读取方式，按文件大小和处理需求选择：

| 方式 | 代码 | 适用场景 |
|------|------|----------|
| 一次性读取 | `os.ReadFile("file.txt")` | 小文件（配置、JSON 等），自动开关文件，最简洁 |
| 逐行读取 | `bufio.NewScanner(file)` | 大文件（日志、CSV 等），内存友好 |
| 流式读取 | `io.ReadAll(reader)` | 网络响应、已打开的文件，通用 Reader 接口 |

### 3.1 一次性读取（小文件推荐）

`os.ReadFile` 自动打开、读取、关闭，一步到位：
...
`bufio.Scanner` 逐行处理，避免一次性加载整个文件到内存：
...
> 注意：`bufio.Scanner` 默认最大行长度为 64KB。文件中有超长行时，需在调用 `Scan()` 前设置 `scanner.Buffer(make([]byte, 0), maxSize)` 增大缓冲区。

### 3.3 使用 io.ReadAll 读取

已有打开的 `io.Reader`（如网络响应、已打开的文件）时使用：
...
`bufio.Writer` 先把数据写入内存缓冲区，攒够再批量写磁盘，显著减少 I/O 次数。**务必在结束前调用 `Flush()`**：
...
用 `os.O_APPEND` 在文件末尾追加，不覆盖已有数据。日志场景的标配写法：
...
Go 没有专门的"文件存在"函数，用 `os.Stat` + `os.IsNotExist` 判断：
...
永远不要用字符串拼接路径（如 `dir + "/" + file`），用 `filepath.Join` 确保跨平台兼容：
...
- `0644`：所有者可读写（6=4+2），组和其他用户只读（4）
- `0755`：目录常用，比 0644 多了执行权限（否则无法进入目录）
- `0` 开头表示八进制，对应 3 组权限：所有者 / 组 / 其他，每组 读(4)+写(2)+执行(1)
